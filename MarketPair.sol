...
contract MarketPair is MarketERC20{
    ...
    function _update(
        ...
        uint112 _reserve1) 
        private {
        require(type(uint112).max > balance0 && type(uint112).max > balance1, 'OVERFLOW');
    ...
    function _mintFee(
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint256 rootK = Math2.sqrtu(uint(_reserve0) *_reserve1);
    ...
    function mint(
        ...
        if (_totalSupply == 0) {
            liquidity = (Math2.sqrtu(amount0*amount1))-(MINIMUM_LIQUIDITY);
           _mint(address(0), MINIMUM_LIQUIDITY); // permanently lock the first MINIMUM_LIQUIDITY tokens
        } else {
            liquidity = Math2.min(amount0*(_totalSupply) / _reserve0, amount1*(_totalSupply) / _reserve1);
        }
    ...
    function swap(
        ...
        require(balance0Adjusted*balance1Adjusted >= uint(_reserve0)*_reserve1*(1000**2), ': K-FACTOR');
        }
